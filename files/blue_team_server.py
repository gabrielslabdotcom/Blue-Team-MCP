...
# Neo4j (optional - for blueteam_neo4j_* tools; Claude-driven graph for SOC investigations)
NEO4J_URI = os.environ.get("NEO4J_URI", "").rstrip("/")
...
# ---------------------------------------------------------------------------
# ─── NEO4J GRAPH (Claude-driven SOC investigations) ───────────────────────
# ---------------------------------------------------------------------------
...
async def _neo4j_run(query: str, params: dict = None) -> tuple[list, str]:
    """Execute a Cypher query. Returns (list of records as dicts, error_msg or "")."""
    if not _neo4j_available():
        return [], "NEO4J_URI and NEO4J_PASSWORD must be set. See config.env."
    try:
...
# Entity type validation for Neo4j queries
_ENTITY_TYPE_SAFE = re.compile(r"^[a-zA-Z][a-zA-Z0-9_]*$")
_ENTITY_VALUE_MAX = 256


class Neo4jFindRelatedInput(BaseModel):
    model_config = ConfigDict(str_strip_whitespace=True, extra="forbid")
    entity_type: str = Field(..., description="Entity type: 'ip', 'host', or 'user'")
    entity_value: str = Field(..., max_length=_ENTITY_VALUE_MAX, description="Value, e.g. IP 1.2.3.4, hostname HYDRA-DC, username Administrator")

    @field_validator("entity_type")
    @classmethod
    def validate_entity_type(cls, v: str) -> str:
        v = v.lower()
        if v not in ("ip", "host", "user"):
            raise ValueError("entity_type must be 'ip', 'host', or 'user'")
        return v


@mcp.tool(
    name="blueteam_neo4j_find_related",
    annotations={"readOnlyHint": True, "destructiveHint": False, "openWorldHint": True}
)
async def blueteam_neo4j_find_related(params: Neo4jFindRelatedInput) -> str:
    """Find alerts and investigations related to an IP, host, or user.
    Requires NEO4J_URI and NEO4J_PASSWORD. Uses the SOC relationship graph.

    Args:
        params.entity_type: ip, host, or user
        params.entity_value: The IP address, hostname, or username to look up

    Returns:
        str: JSON with related alerts and investigations, or error
    """
    entity_type = params.entity_type
    entity_value = params.entity_value.strip()
    if not entity_value:
        return json.dumps({"error": "entity_value cannot be empty"})

    label = {"ip": "IP", "host": "Host", "user": "User"}[entity_type]
    prop = {"ip": "address", "host": "name", "user": "name"}[entity_type]

    query = f"""
    MATCH (e:{label}) WHERE e.{prop} = $val
    OPTIONAL MATCH (e)<-[:OCCURRED_ON|INVOLVES]-(a:Alert)
    OPTIONAL MATCH (a)<-[:ABOUT]-(i:Investigation)
    WITH e,
         [x IN collect(DISTINCT a) WHERE x IS NOT NULL | {{id: x.id, rule_id: x.rule_id, description: x.description, timestamp: x.timestamp}}] AS alerts,
         [x IN collect(DISTINCT i) WHERE x IS NOT NULL | {{id: x.id, notes: x.notes, timestamp: x.timestamp}}] AS investigations
    RETURN e.{prop} AS entity_value, alerts, investigations
    """
    records, err = await _neo4j_run(query, {"val": entity_value})
    if err:
        return json.dumps({"error": err})
    if not records:
        return json.dumps({"entity_type": entity_type, "entity_value": entity_value, "alerts": [], "investigations": [], "message": "Entity not found in graph"})
    r = records[0]
    return json.dumps({"entity_value": r.get("entity_value"), "alerts": r.get("alerts", []), "investigations": r.get("investigations", [])}, indent=2)


class Neo4jRecordInput(BaseModel):
    model_config = ConfigDict(str_strip_whitespace=True, extra="forbid")
    alert_id: str = Field(..., max_length=128, description="Wazuh alert ID or identifier")
    rule_id: Optional[str] = Field(default=None, max_length=32)
    ...
    notes: str = Field(..., max_length=4096, description="Investigation notes and findings")


@mcp.tool(
    name="blueteam_neo4j_record_investigation",
    annotations={"readOnlyHint": False, "destructiveHint": False}
)
async def blueteam_neo4j_record_investigation(params: Neo4jRecordInput) -> str:
    """Record an investigation in Neo4j: create Alert and Investigation nodes, link to Host/IP/User.
    Use after analyzing a Wazuh alert to build the relationship graph for future queries.

    Args:
        params.alert_id: Wazuh alert ID
        params.rule_id: Optional rule ID
        params.description: Optional alert description
        params.host: Hostname involved (optional)
        params.ip: IP address involved (optional)
        params.user: Username involved (optional)
        params.notes: Investigation notes and findings (required)

    Returns:
        str: JSON with created node IDs and status
    """
    inv_id = f"inv-{datetime.utcnow().strftime('%Y%m%d%H%M%S')}-{os.urandom(4).hex()}"
    timestamp = datetime.utcnow().isoformat() + "Z"

    query = """
    MERGE (a:Alert {id: $alert_id})
    SET a.rule_id = $rule_id, a.description = $description, a.timestamp = $timestamp
    WITH a
    CREATE (i:Investigation {id: $inv_id, notes: $notes, timestamp: $timestamp})
    CREATE (i)-[:ABOUT]->(a)
    WITH a, i
    """
    params_dict = {
        "alert_id": params.alert_id,
        "rule_id": params.rule_id or "",
        "description": params.description or "",
        "timestamp": timestamp,
        "inv_id": inv_id,
        "notes": params.notes[:4096],
    }
    clauses = []
    if params.host and params.host.strip():
        clauses.append("MERGE (h:Host {name: $host}) MERGE (a)-[:OCCURRED_ON]->(h)")
        params_dict["host"] = params.host.strip()
    if params.ip and params.ip.strip():
        clauses.append("MERGE (ip:IP {address: $ip}) MERGE (a)-[:INVOLVES]->(ip)")
        params_dict["ip"] = params.ip.strip()
    if params.user and params.user.strip():
        clauses.append("MERGE (u:User {name: $user}) MERGE (a)-[:INVOLVES]->(u)")
        params_dict["user"] = params.user.strip()
    if clauses:
        query += "\n" + "\n".join(clauses)

    _, err = await _neo4j_run(query, params_dict)
    if err:
        return json.dumps({"error": err})
    _audit_log("blueteam_neo4j_record_investigation", {"alert_id": params.alert_id, "inv_id": inv_id}, "")
    return json.dumps({"status": "recorded", "investigation_id": inv_id, "alert_id": params.alert_id}, indent=2)


class Neo4jContextInput(BaseModel):
    model_config = ConfigDict(str_strip_whitespace=True, extra="forbid")
    entity_type: str = Field(..., description="ip, host, or user")
    entity_value: str = Field(..., max_length=_ENTITY_VALUE_MAX)


@mcp.tool(
    name="blueteam_neo4j_get_entity_context",
    annotations={"readOnlyHint": True, "destructiveHint": False, "openWorldHint": True}
)
async def blueteam_neo4j_get_entity_context(params: Neo4jContextInput) -> str:
    """Get full context for an entity: all related alerts, hosts, IPs, users, and investigations.
    Use to understand the complete picture before or during an investigation.

    Args:
        params.entity_type: ip, host, or user
        params.entity_value: The value to look up

    Returns:
        str: JSON with full context graph
    """
    return await blueteam_neo4j_find_related(Neo4jFindRelatedInput(entity_type=params.entity_type, entity_value=params.entity_value))


class Neo4jListInput(BaseModel):
    model_config = ConfigDict(str_strip_whitespace=True, extra="forbid")
    limit: int = Field(default=20, ge=1, le=100, description="Max investigations to return")


@mcp.tool(
    name="blueteam_neo4j_list_investigations",
    annotations={"readOnlyHint": True, "destructiveHint": False, "openWorldHint": True}
)
async def blueteam_neo4j_list_investigations(params: Neo4jListInput) -> str:
    """List recent investigations stored in the SOC graph.
    Shows investigation ID, alert, notes preview, and timestamp.

    Args:
        params.limit: Max investigations to return (default 20, max 100)

    Returns:
        str: JSON with list of investigations
    """
    query = """
    MATCH (i:Investigation)-[:ABOUT]->(a:Alert)
    RETURN i.id AS id, i.notes AS notes, i.timestamp AS timestamp,
           a.id AS alert_id, a.description AS description
    ORDER BY i.timestamp DESC
    LIMIT $limit
    """
    records, err = await _neo4j_run(query, {"limit": params.limit})
    if err:
        return json.dumps({"error": err})
    return json.dumps({"investigations": records, "count": len(records)}, indent=2)
